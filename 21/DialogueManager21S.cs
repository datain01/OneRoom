...
public class DialogueManager21S : MonoBehaviour
{
    public GameObject panelBubblePrefab; // PanelBubble 프리팹을 참조
    public DialogueStell dialogueStell; // DialogueStell 스크립터블 오브젝트를 참조
    ...
    {
        if (dialogueStell.dialogues.Count == 0)
        {
            Debug.LogError("대사 리스트가 비어 있습니다.");
            return "";
        }
        int randomIndex = Random.Range(0, dialogueStell.dialogues.Count);
        return dialogueStell.dialogues[randomIndex];
    }
    ...
    private void DisplayBubble(string dialogue)
    {
        ...
        if (bubbleFollow != null)
        {
            ...
            bubbleFollow.targetTag = "CharacterSec"; // 적절한 태그를 설정합니다.
        }
