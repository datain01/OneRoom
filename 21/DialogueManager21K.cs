...
public class DialogueManager21K : MonoBehaviour
{
    public GameObject panelBubblePrefab; // PanelBubble 프리팹을 참조
    public DialogueKain dialogueKain; // DialogueKain 스크립터블 오브젝트를 참조
    ...
    {
        if (dialogueKain.dialogues.Count == 0)
        {
            Debug.LogError("대사 리스트가 비어 있습니다.");
            return "";
        }
        int randomIndex = Random.Range(0, dialogueKain.dialogues.Count);
        return dialogueKain.dialogues[randomIndex];
    }
    ...
    private void DisplayBubble(string dialogue)
    {
        ...
        // 캐릭터 위에 위치하도록 설정
        Vector3 offset = new Vector3(1.1f, 1.4f, 0f); // UI 오프셋
