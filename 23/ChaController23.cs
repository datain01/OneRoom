...
public class ChaController23 : MonoBehaviour
{
    ...
    public DialogueManager23 dialogueManager;
    public DialogueManagerSec23 dialogueManagerSec;
    ...
    private void OnMouseUp()
    {
        if (!isDragged && !isLiked && !isEating)
        {
            ...
            {
                // 적절한 대화 관리자를 선택하여 대사 출력
                if (gameObject.CompareTag("CharacterOne"))
                {
                    dialogueManager.DisplayRandomDialogue();
                }
                else if (gameObject.CompareTag("CharacterSec"))
                {
                    dialogueManagerSec.DisplayRandomDialogue();
                }
            }
        }
        else
    ...
    // IsWithinBoundary 메서드는 하나만 유지합니다.
    ...
    // 외부에서 멈추는 기능을 호출할 때 Idle 상태로 설정하는 메서드 추가
